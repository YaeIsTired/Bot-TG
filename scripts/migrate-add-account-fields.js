...
const addAccountFields = () => {
    const fieldsToAdd = [
        ...
        { name: 'bind_info', type: 'TEXT' },
        { name: 'country', type: 'TEXT' },
        { name: 'creation_date', type: 'DATE' },
        { name: 'banned_status', type: 'TEXT' },
            ...
            if (!columnExists) {
                db.run(`ALTER TABLE accounts ADD COLUMN ${field.name} ${field.type}`, (err) => {
                    ...
                    if (completedFields === totalFields) {
                        ...
                        console.log('All new account fields have been added to the database.');
                        db.close();
                    }
                });
            } else {
...
console.log('🚀 Starting migration to add new account fields...\n');
addAccountFields();
